...
docker run -dit --name firewall --privileged \
...
# pass default gateway to client1

# Client 1
docker run -dit --name client1 --privileged \
...
# Client 2
docker run -dit --name client2 --privileged \
  --network labnet172 --ip 172.24.0.2 \
  $PORTS_CLIENT2 \
  ubuntu-lab

# Client 3
docker run -dit --name client3 --privileged \
